            ...
            objectKeys.forEach(async (key) => {
                ...
                if (Array.isArray(element[key])) {
                    ...
                    if (!this.alreadyCreatedObjects[path + "." + key]) {
                        ...
                        if (element.stringListKeyValues && (key === "valueKey" || key === "value")) {
                            common.states = element.stringListKeyValues;
                        }

                        if (key === "valueKey" || (key === "editable" && element[key] === true)) {
                            ...
                            if (element.stringListKeyValues) {
                                commonSetValue.states = element.stringListKeyValues;
                            }

                            await this.adapter.setObjectNotExistsAsync(setStatePath, {
                                type: "state",
                                common: commonSetValue,
                                native: {},
                            });
                        }
                        await this.adapter
    ...
    extractArray(element, key, path, options) {
        try {
            ...
            element.forEach(async (arrayElement, index) => {
                ...
                if (options.preferedArrayName.indexOf("+") !== -1) {
                ...
                } else if (options.preferedArrayName.indexOf("/") !== -1) {
                    const preferedArrayNameArray = options.preferedArrayName.split("/");
